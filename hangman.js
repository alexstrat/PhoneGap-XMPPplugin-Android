...
var Guesser = function(xmpp) {
  ...
  var self = this;
  xmpp.addHandler(function(response) {
    self.handler(response);
    return true;
  }, null, "message", "chat");
};

Guesser.prototype.start = function(opponent) {
  this.tried    = [];
  this.opponent = opponent;
  var json = { start: this.jid };
  var stanza = $msg({to: opponent, from: this.jid, type: "chat"})
               .c("body")
               .t(JSON.stringify(json));
  this.xmpp.send(stanza);
};

Guesser.prototype.play = function(letter) {
  letter = String(letter).charAt(0).toUpperCase();
  if (~this.tried.indexOf(letter)) {
    throw new Error("already tried");
  }
  this.tried.push(letter);
  var json = { letter : letter };
  var stanza = $msg({to: this.opponent, from: this.jid, type:"chat"})
                .c("body")
                .t(JSON.stringify(json));
  this.xmpp.send(stanza);
};
...
Guesser.prototype.finish = function(result) {
  this.emit("finish", result);
};

var Master = function(xmpp) {
  EventEmitter.call(this);
  ...
  var self = this;
  this.xmpp.addHandler(function(request) {
    self.handler(request);
    return true;
  }, null, "message", "chat");
};

Master.prototype.start = function(word) {
  this.word  = String(word).toUpperCase();
  this.tries = 0;
  this.extractWord(this.word);
};

Master.prototype.extractWord = function(word) {
  var firstLetter = word.charAt(0);
  var lastLetter = word.charAt(word.length - 1);
  this.letters = [];
  this.found   = [];
  this.wordToFound = Array(word.length);
  for (var i = 1; i < word.length; i++) {
    var char = word.charAt(i);
    if (char === lastLetter) {
      this.wordToFound[i] = lastLetter;
    } else if (char === firstLetter) {
      this.wordToFound[i] = firstLetter;
    } else {
      this.wordToFound[i] = null;
    }
    if (this.letters.indexOf(char) === -1 && char !== lastLetter && char !== firstLetter) {
...
Master.prototype.handler = function(request) {
  try {
    ...
    if (request.letter) {
      ...
      this.handleStart(request.start);
    }
  } catch(e) {
    console.error(e);
  }
...
Master.prototype.handleStart = function(opponent) {
  this.opponent = opponent;
  var json = { 
    type : "start",
    word : this.wordToFound
  };
  var stanza = $msg({to: opponent, from: this.jid, type: "chat"})
               .c("body")
               .t(JSON.stringify(json));
...
Master.prototype.handleLetter = function(letter) {
  ...
  if (this.found.indexOf(letter) >= 0) return;
  
  var response;
  if (this.letters.indexOf(letter) !== -1) {
    this.found.push.apply(this.found, this.letters.splice(this.letters.indexOf(letter), 1));
    var positions = this.extractPositions(letter);
    response = {
      type      : "success",
      positions : positions,
      letter    : letter,
      finish    : (this.letters.length === 0)
    };
  } else {
    this.tries++;
    response = {
...
for(var i in EventEmitter.prototype) { Guesser.prototype[i] = EventEmitter.prototype[i]; }
for(var i in EventEmitter.prototype) { Master.prototype[i]  = EventEmitter.prototype[i];  }